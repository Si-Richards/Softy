...
const Dialpad = () => {
  ...
  return <div className="w-full max-w-md mx-auto p-6">
      ...
      <NumberInput number={number} onChange={setNumber} onClear={clearNumber} onBackspace={handleBackspace} disabled={isCallActive} // Disable editing number input during calls
    />

      <DialpadGrid onKeyPress={handleKeyPress} isCallActive={isCallActive} />

      <CallControls isCallActive={isCallActive} muted={muted} number={number} onCall={() => handleCall(number, isJanusConnected)} onToggleMute={toggleMute} onCallVoicemail={callVoicemail} />
      
      {/* Updated shortcode options with tooltips */}
      {!isCallActive && <div className="mt-6">
          
          <div className="grid grid-cols-3 gap-2">
            {shortcodes.map((item, index) => <Tooltip key={index}>
                <TooltipTrigger asChild>
                  <Button variant="outline" className="flex flex-col items-center justify-center h-16 p-2 border border-gray-200 rounded-md hover:bg-gray-50" onClick={() => setShortcode(item.code)} disabled={!isJanusConnected}>
