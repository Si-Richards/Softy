...
const SipCredentialsForm: React.FC<SipCredentialsFormProps> = ({
  ...
  return <form onSubmit={handleFormSubmit} className="space-y-4">
      <div className="space-y-2">
        <Label htmlFor="username">Username</Label>
        <Input id="username" value={username} onChange={e => setUsername(e.target.value)} placeholder="Enter your SIP username" disabled={isDisabled || isReadOnly} className={isReadOnly ? "bg-gray-100" : ""} readOnly={isReadOnly} />
      ...
      <div className="space-y-2">
        ...
        <Input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your SIP password" disabled={isDisabled || isReadOnly} className={isReadOnly ? "bg-gray-100" : ""} readOnly={isReadOnly} />
      </div>
      <div className="space-y-2">
        <Label htmlFor="host">SIP Host</Label>
        <Input id="host" value={internalSipHost} onChange={e => handleSipHostChange(e.target.value)} placeholder="SIP Server Address" disabled={isDisabled || isReadOnly} className={isReadOnly ? "bg-gray-100" : ""} readOnly={isReadOnly} />
        <p className="text-xs text-gray-500">
          Format: hostname:port (e.g., "hpbx.sipconvergence.co.uk:5060")
        </p>
      </div>
      <div className="pt-4 flex gap-2 flex-wrap">
        <Button type="submit" disabled={isDisabled || !username || !password} className="w-full sm:w-auto">
          {buttonText}
        </Button>
        
        <Button type="button" variant="outline" onClick={handleDebugInfo} className="w-full sm:w-auto">Check WebRTC Status</Button>
      </div>
    </form>;
};
